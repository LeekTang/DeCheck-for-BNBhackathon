<template>
  <div class="w-[75rem] mx-auto mt-[4rem]" v-if="Object.keys(state.goInfo).length > 0">
    ...
    <div class="flex justify-between mt-[1.5rem]">goInfo
      <div class="w-[36.75rem] h-[31.5rem] rounded-[1.25rem] basic-bg">
        ...
        <div class="p-[1.5rem] text-[0.88rem]">
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
            ...
            <p class="text-[#FFFFFF] font-bold w-[18rem]">{{state.goInfo.token_symbol}} ({{state.goInfo.token_name}})</p>
          </div>
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
            <p class="text-[#FFFFFFA8]">Token Contract Address</p>
            <p class="text-[#FFFFFF] font-bold w-[18rem]" v-if="state.goInfo.owner_address">{{plusXing(state.goInfo.owner_address)}}</p>
          </div>
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
            <p class="text-[#FFFFFFA8]">Contract Creator</p>
            <p class="text-[#FFFFFF] font-bold w-[18rem]" v-if="state.goInfo.creator_address">{{plusXing(state.goInfo.creator_address)}}</p>
          </div>
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
          ...
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
            ...
            <p class="text-[#FFFFFF] font-bold w-[18rem]">2023-04-23 22:11:56 UTC</p>
          </div>
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
      ...
      <div class="w-[36.75rem] h-[31.5rem] rounded-[1.25rem] basic-bg">
        ...
        <div class="p-[1.5rem] text-[0.88rem]">
          <div class="h-[3.5rem] flex justify-between items-center border-b-2 border-b-[#FFFFFF1C]">
            ...
            <p class="text-[#FFFFFF] text-[1.5rem] font-bold">5.56$</p>
          </div>
          ...
          <div class="flex justify-between">
            <div class="h-[17.5rem] w-[16.13rem] bg-[#ffffff1c] rounded-[0.75rem] p-[1rem] flex flex-col justify-around">
              ...
              <div class="flex justify-between">
                ...
                <p class="text-[1.5rem] text-[#11B466] font-bold">0.29%</p>
              </div>
            </div>
            <div class="h-[17.5rem] w-[16.13rem] bg-[#ffffff1c] rounded-[0.75rem] p-[1rem] flex flex-col justify-around">
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <p class="text-[1.5rem] text-[#FF5353FF] font-bold">0.29%</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="h-[40rem] w-full basic-bg rounded-[1.25rem] mt-[1.5rem]">
...
const riskList = [
  {state: 1, text: 'Doesn’t look like honeypot'},
  {state: 1, text: 'No blacklist'},
  {state: 1, text: 'Contract is open source'},
  {state: 1, text: 'Admin privileges abandoned'},
  {state: 2, text: 'Owner can not tamper with balance'},
  {state: 1, text: 'Can not Mint'},
  {state: 3, text: 'Doesn’t look like a proxy contract'},
  {state: 1, text: 'Can not take back ownership'},
  {state: 1, text: 'Doesn’t look like honeypot'},
  {state: 1, text: 'No trading - cool - down mechanism'},
  {state: 1, text: 'Slippage cannot be modified'},
  {state: 1, text: 'Trading is active'},
  {state: 1, text: 'No whitelist'},
  {state: 1, text: 'No anti whale'},
]

const state = reactive({
  goInfo: computed(() => store.getSearchInfo),
})
